...
def _ensure_category_vectors():
    ...
    with _LOCK:
        if _category_vectors is not None:
            return
        try:
            vecs = _embed([c["keywords"] for c in CORE_CATEGORIES.values()])
...
def classify_category_by_embedding(text: str) -> dict:
    ...
    if not text:
        key = _FALLBACK_CATEGORY
        meta = CORE_CATEGORIES.get(key, {})
        return {
            "key": key,
            "en": meta.get("en", key.capitalize()),
            "zh-TW": meta.get("zh-TW", key),
        }
    ...
    key = best_cat if best_sim >= _SIM_THRESHOLD else _FALLBACK_CATEGORY
    meta = CORE_CATEGORIES.get(key, {})
    return {
        "key": key,
        "en": meta.get("en", key.capitalize()),
